...
- 主要 JSON（`config/app.json` でパスをまとめて指定）:
  - `assets/game.json` … 固定デルタ (`fixed_dt=1/60`)、ピクセル単位、スポーン・リスポーン、LOD、性能予算。
  - `assets/entities.json` … コマンダー／ちび／敵の基礎ステータス。
  - `assets/jobs.json` / `assets/skills.json` … 職業ごとのスキル、共通クールダウン、攻撃仕様。
  - `assets/morale.json` / `assets/chibi_personality.json` / `assets/ai_params.json` / `assets/ai_temperaments.json` … 士気や性格、行動パラメータ。
  - `assets/formations.json` … 陣形（align 時間・被ダメ倍率）。
  - `assets/spawn_level1.json` … 時間ウェーブ通知と敵セット（A ゲートなど）。
  - `assets/stage1_config.json` … 味方/敵拠点の座標・HP・湧きレート・勝利条件、ステージ Hazard。
  - `assets/map_defs.json` と `assets/maps/level1.tmx` … マップ衝突と表示。
  - `assets/atlas.json` / `assets/ui/*` … スプライトアトラスと UI 素材。
  - `assets/economy.json` / `assets/camp_upgrades.json` / `assets/training.json` / `assets/meta_shop.json` / `assets/strategies.json` … キャンプ強化・ショップ・訓練・メタ通貨。
...
- ミッション要素: `assets/mission_level1.json` のボス（ドラゴン）や、`stage1_config.json` の拠点封鎖・Hazard 発火条件を `StageConfig` として `LegacySimulation` が扱う。
...
- ちびわふ（Unit）: 職業タグ付き。スポーン間隔/上限、職業重み、ピティ設定は `assets/game.json` と `assets/spawn_weights.json`。性格と士気で Panic（攻撃停止・退避）や Mesomeso 状態に入る。1 体ごとに職業・称号・性格の個性を持つ。
- 敵（EnemyUnit）: スライム/ゴブリン/マジシャン/バット/トリトリ/ゴーレム/ウォールブレイカー/ドラゴンなど。行動はシンプルなターゲット直進＋一部特性（壁優先、カイト、拠点特攻）。`mission_level1.json` ではドラゴンの大扇形炎とスラムを定義。
- 拠点/壁: `stage1_config.json` の味方拠点にオーラ（与ダメ↑/被ダメ↓/HP 再生）を付与。`SelfDestruct` などの壁生成は `Wall` スキル経由でセグメントを生成し、Wallbreaker は半径内の壁を優先。
...
- パフォーマンス監視: `performance.budget`（`assets/game.json`）をもとに `PerformanceBudgetMonitor` が入力/更新/描画/HUD を計測し、超過すると HUD テキストで警告。
...
- データ定義: `assets/*.json`, `assets/maps/level1.tmx`
- キャンプ/メタ: `src/game/*`, `assets/camp_upgrades.json`, `assets/training.json`, `assets/meta_shop.json`
...
## 12. スポーンとウェーブ詳細
- 味方スポーン:
  - レート/上限: `assets/game.json.spawn` の `yuna_interval_s`（0.75s）、`yuna_max`（200）。スポーン位置は拠点口＋`yuna_offset_px`（48,0）に Y ばらつき ±`yuna_scatter_y_px`（16）。
  - 職業抽選: `spawn.weights` と `spawn_weights.json`。同職 3 連続後は未出職重み×2 のピティ。`history_limit` で履歴を管理し偏りを抑制。
  - スポーン予算: `SpawnBudgetConfig.maxPerFrame`（デフォルト 8）で 1 フレームに生成する数を制限し、超えた分はキュー繰り越し。警告テキストは `warning_text`。
- 敵スポーン:
  - `assets/spawn_level1.json` に時間 `t`（秒）ごとのセットとテレメトリ文言。ゲートはタイル座標で指定（例: A=(2,10)）。
  - `assets/stage1_config.json.enemy_bases` に常時湧きのレート (`rate_per_s`, `rate_max`) と各敵の重み。`global_caps.enemies` で同時出現数を制限。
  - 拠点封鎖: `on_base_sealed.stop_spawn=true` で封鎖後のスポーンを止め、当たり判定を外して演出だけ残す。
- ボス: `assets/mission_level1.json` でドラゴンの HP/速度/半径/スキル周期などを定義。UI 表示フラグ（ボス HP バーやタイマー）もここ。

## 13. AI・性格・士気のもう少し踏み込み
- 性格（`chibi_personality.json`）:
...
- AI 判定（`BehaviorSystem`）:
  ...
  - Boids 味付け: 半径 96px 重心への Cohesion、18px 以内で Separation。`ai_params.json` の重みで調整。
...
## 14. 号令・陣形・スキル
- 号令（10 秒間の行動上書き。`skills.json` で継続時間や表示名を持つ）:
  - 突撃: 最寄りの敵へ突進。前線を押し上げたい時。
  - 前進: ウェイポイントへ進軍、遭遇戦闘。
  - 追従: コマンダー追従（上限 30 体）。フォーメーションが適用される。
  - 防衛: 拠点周囲で迎撃。
- 陣形（`formations.json`）: Aligning 期間中は被ダメ倍率が上がる代わりに並びを揃える。Swarm/Wedge/Line/Ring を Z/X で巡回。
- コマンダースキル例:
  - FireBall (`FireBall` 入力): 単体ダメージ、射程とダメージは `skills.json`。
  - Guard (`K`): 受けるダメージを軽減しながら前進。持続と軽減率は `skills.json`。
  - SelfDestruct/Wall/Surge/Rally: `skills.json` のクールダウンと効果で制御。自己破壊はリスポーンペナルティが倍になるが命中数で短縮。
...
- アトラス: `assets/atlas.json` と `assets/atlas.png` でスプライトを管理。`RenderingPrepSystem` で Y ソート後に SDL_Renderer へ描画。
- ビネット: 画面全体に暗めの矩形オーバーレイ。LOD 発動時は隔フレームで描画スキップ。
- フォールバック: アセットロード失敗時は 1px 透明テクスチャ・NotoSans フォント・空 JSON を返し、テレメトリに警告を出す。

## 17. データを触るときのガイド
- 動作確認しやすい順: `assets/game.json`（時間系/リスポーン/LOD）→ `assets/spawn_level1.json`（ウェーブ）→ `assets/stage1_config.json`（拠点/敵湧き）→ `assets/entities.json`（HP/速度）→ `assets/jobs.json`/`skills.json`（クールダウンや倍率）。
...
| 項目 | 値 | 参照 |
| --- | --- | --- |
| コマンダー HP / DPS / 速度 | 60 / 15 / 3.2 u/s | `assets/entities.json.commander` |
| ちび HP / DPS / 速度 | 10 / 3 / 1.8 u/s | `assets/entities.json.yuna` |
| ちびスポーン | 0.75s 間隔 / 上限200 / ばらつき ±16px | `assets/game.json.spawn` |
| スライム HP / DPS / 速度 | 80 / 5 / 0.9 u/s | `assets/entities.json.enemies.slime` |
| ウォールブレイカー HP / DPS(壁15) / 速度 | 60 / 15 / 1.0 u/s | `assets/entities.json.enemies.wallbreaker` |
| ドラゴン HP / 速度 / 半径 | 2250 / 0.8 u/s / 40px | `assets/mission_level1.json.boss` |
| 号令時間 | 10s | `assets/skills.json.commands.*.duration` |
| 陣形 align 時間 / 被ダメ倍率 | 例: 1.0s / 1.15 など | `assets/formations.json` |
| LOD | 300 体超で skip_draw_every=2 | `assets/game.json.lod` |
| リスポーン | ちび 5〜20s、コマンダー 12s〜 | `assets/game.json.respawn` |

（細かい攻撃範囲やクールダウンは `assets/skills.json` / `assets/jobs.json` を参照）
...
- 体験版コンセプトの「無限湧き＋拠点封鎖で短縮」は `stage1_config` で制御、`spawn_level1.json` は簡易ウェーブ通知のみ。
- 自動セーブはキャンプ状態のみ。ラン中の途中セーブ/リジュームは未対応。

## 22. デザイナー/SE向けチューニング手順（短縮版）
1. スポーン・敵編成を変える: `assets/stage1_config.json`（常時湧きと拠点）→ `assets/spawn_level1.json`（時間セット）。`F9` でホットリロード、`Shift+F10` で TSV ダンプ確認。
2. バランスを動かす: HP/速度/DPS は `assets/entities.json`。職業・スキル倍率は `assets/jobs.json` / `assets/skills.json`。変更後に `F9`。
3. 性格/士気/AI を調整: `assets/chibi_personality.json` / `assets/morale.json` / `assets/ai_params.json`。Panic しきい値やターゲットロック時間を確認。
4. パフォーマンスを見る: `F10` で HUD を出し、Budget 警告が出たら `assets/game.json.performance` 目標を見直すかスポーン数を抑える。
5. レンダリング/LOD を触る: `assets/renderer.json`（sRGB, pixel_snap）、`assets/game.json.lod`（描画間引き）。
